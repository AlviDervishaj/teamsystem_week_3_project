...
export const Education = () => {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const prevState: {fName: string, lName: string, pNumber: string} = location.state;
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-lg mx-auto bg-slate-100 p-4 mt-20 rounded-md">
      <Input title="University Name" placeholder="University Name" />
      <Input title="Start Year" placeholder="Start Year" />
      <Input title="Graduated Year" placeholder="Graduated Year" />
      <Input title="Type" placeholder="Bachelor or Master" />
